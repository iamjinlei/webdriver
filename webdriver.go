...
func (c Capabilities) AddChrome(f chromeCapabilities) {
	c[CapabilitiesKey] = f
	c[DeprecatedCapabilitiesKey] = f
}
...
func (c Capabilities) AddLogging(l LogCapabilities) {
	c[LogCapabilitiesKey] = l
}

// SetLogLevel sets the logging level of a component. It is a shortcut for
// passing a LogCapabilities instance to AddLogging.
func (c Capabilities) SetLogLevel(typ LogType, level LogLevel) {
	if _, ok := c[LogCapabilitiesKey]; !ok {
		c[LogCapabilitiesKey] = make(LogCapabilities)
	}
	m := c[LogCapabilitiesKey].(LogCapabilities)
	m[typ] = level
}
